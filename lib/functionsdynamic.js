...
function parseLayout(loadLayout = {})
{
	var sidebar = document.getElementById("top-section");
	if(loadLayout.length == 0)
	{
	...
	{
		var sandwich = `<a style="margin-top: 10px;" class="TYPEHERE-link" id="TYPEHERE-sel">
					<span class="fa-stack fa-fw fa-lg">
						<i class="CLASSHERE"></i>
						<span class="tooltiptext">TYPEHERE</span>
					</span>
		</a>`;
		sandwich = sandwich.replace("CLASSHERE", layout[section].icon_class);
		sandwich = sandwich.replace("TYPEHERE", section);
		sidebar.insertAdjacentHTML('beforeend', sandwich);
