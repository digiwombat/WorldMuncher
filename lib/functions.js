...
function newWorld(newWorld = true)
{
	...
	if(newWorld == true)
	{
		zip.file('userDB.json', JSON.stringify(newDB));	zip.generateNodeStream({type:'nodebuffer',compression:"DEFLATE",streamFiles:true}).pipe(fs.createWriteStream(worldDBLocation)).then(loadWorld());
	}
	else
	{
		loadWorld();
