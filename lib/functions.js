...
const medium = require('medium-editor');
...
var editor = new medium.MediumEditor('.editable', {
    toolbar: {
...
function loadPlaceList()
{
	...
	$(placeList).empty();
	_.orderBy(worlddb['places'], 'id', 'asc').forEach(function(row) {
		console.log("Selected Rows.");
		var li = document.createElement('li');
		...
		if(row.place_parent)
		{
			$('#place-' + row.place_parent).append(li);
...
function loadPerson()
{
	...
	$('.editable').textcomplete([{
		...
		replace: function (word) {
			return '<a contenteditable="false" href="#' + word.listType + '/' + word.listID + '" class="' + word.listType + '-link">' + word.listValue + '</a> ';
		},
...
function loadPlace()
{
	...
	for (var key in place) {
		if (place.hasOwnProperty(key) && key != 'place_picture' && document.getElementById(key) !== null) {
...
function addPerson()
{
	...
	var newPerson = {
		...
		"person_description": "<p></p>",
		"person_personality": "<p></p>",
		"person_background": "<p></p>",
		"person_status": "",
