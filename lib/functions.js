...
function init()
{
	...
	}).error(function() { $('#vercheck').html('WorldMuncher v' + pjson['version'] + ' | Update Check Failed'); });
...
//#region Load List Functions
function loadPersonList()
{
	var peopleList = document.getElementById('list-holder');
	$(peopleList).empty();
	_.orderBy(worlddb['people'], 'person_name', 'asc').forEach(function(row) {
	...
	$('#filter').attr('placeholder', 'People');
...
function loadPlaceList()
{
	...
	$(placeList).empty();
	_.orderBy(worlddb['places'], 'place_name', 'asc').filter(function (el) {
	...
	{
		_.orderBy(worlddb['places'], 'place_name', 'desc').filter(function (el) {
	...
	{
		$('#place-' + selectedID).addClass('list-active');
		loadPerson();
	}

	makeLists();

	$('#filter').attr('placeholder', 'Places');
...
function loadThingList()
{
	...
	$(thingList).empty();
	_.orderBy(worlddb['things'], 'thing_name', 'asc').forEach(function(row) {
		var li = document.createElement('li');
	...
	$('#filter').attr('placeholder', 'Things');
	$('#people').removeClass('active');
	$('#places').removeClass('active');
...
{
	var noteList = document.getElementById('list-holder');
	$(noteList).empty();
	_.orderBy(worlddb['notes'], 'note_name', 'asc').forEach(function(row) {
		var li = document.createElement('li');
		li.setAttribute('id', 'note-' + row.note_id);
		li.classList.add('note');
		var pic = '';
		li.innerHTML = `<div class="note-image"><div></div></div>
			<h3>` + row.note_name + `</h3>`;
		noteList.appendChild(li);
	});

	var anchors = document.querySelectorAll("li.note");
	for (var i = 0; i < anchors.length; i++) {
		var current = anchors[i];
		current.addEventListener('click', function() {
			var noteID = this.getAttribute('id').replace('note-', '');
			selectedIDType = "note";
			selectedID = noteID;
			loadNote();
			$('.list-active').removeClass('list-active');
	...
	{
		$('#note-' + selectedID).addClass('list-active');
		loadNote();
	}

	makeLists();

	$('#filter').attr('placeholder', 'Notes');
	$('#people').removeClass('active');
	$('#places').removeClass('active');
	$('#things').removeClass('active');
...
function loadPlace()
{
	...
	$('#place_parent').on('change', function(e){
		...
		{
			if(place['place_parent'] == '')
			{
				var children = worlddb['places'].filter(function(row){ return row.place_parent == place['place_id']});
				children.forEach(function(row){ row.place_parent = ''})
			}
