...
function init()
{
	...
	$('.exit-button').on('click',function(){
		...
		if(worlddb && needsSave == 1)
		{
			// let options = {
			// 	buttons: ["Yes", "No"],
			// 	message: "Unsaved Changes. Would you like to save before closing?"
			// }
			// dialog.showMessageBox(window, options, (res, checked) =>
			// {
			// 	if (res === 0) 
			// 	{
			// 		//Yes button pressed
			// 		console.log("Yes");
			// 		saveZip(true).then(window.close());
			// 	}
			// 	else if (res === 1)
			// 	{
			// 		console.log("No");
...
function loadList(type)
{
	...
	$('#list-holder li').on('cssClassChanged', function(){
		...
		var item = worlddb[plural].filter(function (el) {
	...
	"people":
		{
			...
			"fields": {
				"id": { "row": 1, "columns": 1, "type": "display" },
				"name": { "row": 1, "columns": 11, "type": "text", "default": "New Person" },
				"age": { "row": 2, "columns": 6, "type": "text", "default": "Unknown" },
				"gender": { "row": 2, "columns": 6, "type": "text", "default": "Unknown" }
			}
		}
}
